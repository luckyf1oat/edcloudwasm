...
const SSAEAD标签长度 = 16;
const SSNonce长度 = 12;
const SS子密钥信息 = textEncoder.encode('ss-subkey');
...
const SS递增Nonce计数器 = (counter) => {
    for (let i = 0; i < counter.length; i++) {
        counter[i] = (counter[i] + 1) & 0xff;
        if (counter[i] !== 0) break;
    }
};
const SS派生主密钥 = async (passwordText, keyLen) => {
...
const SS派生会话密钥 = async (config, masterKey, salt, usages) => {
...
const SSAEAD加密 = async (cryptoKey, nonceCounter, plaintext) => {
...
const 创建SS入站解密器 = (config, passwordText) => {
    ...
    return {
        async 输入(dataChunk) {
            ...
            if (!入站状态.hasSalt) {
                if (入站状态.buffer.byteLength < config.saltLen) return plaintextChunks;
                const salt = 入站状态.buffer.subarray(0, config.saltLen);
                入站状态.buffer = 入站状态.buffer.subarray(config.saltLen);
                const masterKey = await SS派生主密钥(passwordText, config.keyLen);
                入站状态.decryptKey = await SS派生会话密钥(config, masterKey, salt, ['decrypt']);
                入站状态.hasSalt = true;
            }
            while (true) {
...
const 创建SS出站加密器 = async (config, passwordText) => {
    const 出站主密钥 = await SS派生主密钥(passwordText, config.keyLen);
    const 出站盐 = crypto.getRandomValues(new Uint8Array(config.saltLen));
    const 出站加密密钥 = await SS派生会话密钥(config, 出站主密钥, 出站盐, ['encrypt']);
...
const handleWebSocketConn = async (webSocket, request) => {
    ...
    const 请求加密方式 = (requestURL.searchParams.get('enc') || '').toLowerCase();
    const SS加密配置 = SS支持加密配置[请求加密方式];
    const 启用SSAead模式 = !!SS加密配置;
    const protocolHeader = request.headers.get('sec-websocket-protocol');
    // @ts-ignore
    const earlyData = (!启用SSAead模式 && protocolHeader) ? Uint8Array.fromBase64(protocolHeader, {alphabet: 'base64url'}) : null;
    ...
    if (启用SSAead模式) {
        const ss解密器 = 创建SS入站解密器(SS加密配置, uuid);
        let ss出站加密器 = null;
        ...
        const ss回包Socket = {
            send(data) {
                ...
                SS发送队列 = SS发送队列.then(async () => {
                    ...
                    if (!ss出站加密器) ss出站加密器 = await 创建SS出站加密器(SS加密配置, uuid);
                    const encrypted = await ss出站加密器.加密(chunk);
                    if (encrypted.byteLength) webSocket.send(encrypted);
                }).catch(() => close());
            }
        };
        const processSS = async (chunk) => {
            const 明文块数组 = await ss解密器.输入(new Uint8Array(chunk));
            for (let i = 0; i < 明文块数组.length; i++) {
                const 明文块 = 明文块数组[i];
                if (state.tcpWriter) {
                    await state.tcpWriter(明文块);
                } else {
                    await handleSession(明文块, state, request, ss回包Socket, close);
                }
            }
        };
        webSocket.addEventListener("message", event => {processingChain = processingChain.then(() => processSS(event.data).catch(close))});
        return;
    }
